...
public class KaratResource {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(KaratResource.class);
    ...
    public ResponseEntity<Karat> createKarat(@Valid @RequestBody Karat karat) throws URISyntaxException {
        ...
        karat = karatService.save(karat);
        return ResponseEntity.created(new URI("/api/karats/" + karat.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, karat.getId().toString()))
            .body(karat);
    }
    ...
    public ResponseEntity<Karat> updateKarat(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Karat karat)
        ...
        karat = karatService.update(karat);
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<Karat>> getAllKarats(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Karat> getKarat(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteKarat(@PathVariable("id") Long id) {
