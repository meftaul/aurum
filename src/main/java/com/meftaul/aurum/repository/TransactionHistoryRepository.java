...
public interface TransactionHistoryRepository
    ...
    @Query(
        value = "SELECT tag, SUM(amount) AS totalAmount" +
        " FROM transaction_history" +
        " WHERE date_created BETWEEN :startDate AND :endDate" +
        " GROUP BY tag",
        nativeQuery = true
    )
    List<TransactionHistoryAmountByTag> totalAmountByTag(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);

    @Query(
        value = "select customer_id as customerId, tag, sum(amount) as totalAmount from transaction_history " +
        " where date_created between :startDate and :endDate and customer_id = :customerId" +
        " group by tag, customer_id",
        nativeQuery = true
    )
    List<TransactionHistoryAmountByTagAndCustomer> totalAmountByTagAndCustomerId(
    ...
    @Query(
        value = "select customer_id as customerId, sum(amount) as totalAmount from transaction_history" +
        " where date_created between :startDate and :endDate" +
        " group by customer_id order by totalAmount desc limit :n",
        nativeQuery = true
    )
